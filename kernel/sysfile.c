...
sys_mmap(void){
    ...
    if(argaddr(0,&va_addr)<0 || argint(1,&length)<0 || argint(2,&prot)<0 ||argint(3,&flags)<0 ||argint(4,&fd)<0 ||argint(5,&offset) <0)
        return -1;
    struct proc* p=myproc();
    struct file* mmapfile=p->ofile[fd];
    if ((!mmapfile->writable)&&(prot&PROT_WRITE)&&(!(flags&MAP_PRIVATE)))
        return -1;
    int i;
    for (i = 0; i < NVMA; i++) {
        // allocate vma struct
...
sys_munmap(void)
{
    ...
    if (vmap->flags & MAP_SHARED) {
        filewrite(vmap->mmapfile, addr, length);
    }
