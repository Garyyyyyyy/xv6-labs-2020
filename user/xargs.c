...
int readline(int nargc, char *nargv[32]){
    char buf[1024];
    int n = 0;
...
//        printf("%s\n",argv[i]);
        ...
        memcpy(nargv[i-1],argv[i], strlen(argv[i]));
    }

    while((nargc=readline(argc-1,nargv))!=0){
        nargv[nargc]=0;
        if (fork()==0) exec(argv[1],nargv);
        else wait(0);
    }
    exit(0);
}
